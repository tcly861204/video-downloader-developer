...
fn sanitize_ks_json(raw: &str) -> String {
    // 1. 处理嵌套的未转义 JSON 字符串值
    //    例如: "atlasList":"{"list":[...]}" → 无法直接解析
    //    策略: 找到这类模式，把内层 JSON 作为字符串值处理
    let mut sanitized = raw.to_string();

    // 修复常见问题: 值中的未转义引号
    // 模式: "key":"{  或  "key":"[  或  }","url"
    // 快手 INIT_STATE 中偶有这类格式问题

    // 2. 移除 JSON 中的 JS 函数引用
    //    如: function(){return ...} 或 ;(")
    if let Ok(re) = Regex::new(r#"function\s*\([^)]*\)\s*\{[^}]*\}"#) {
        sanitized = re.replace_all(&sanitized, "null").to_string();
    }

    // 3. 处理 trailing commas (JSON 规范不允许)
...
fn extract_video_from_json(data: &Value, photo_id: &str) -> Option<VideoInfo> {
    ...
    // 结构: {"defaultClient": {"VisionVideoDetailPhoto:xxx": {...}, "VisionVideoDetailAuthor:xxx": {...}}}
...
/// 直接从 {"defaultClient": {"VisionVideoDetailPhoto:xxx": {...}}} 提取
...
fn find_photo_in_json(value: &Value, photo_id: &str) -> Option<VideoInfo> {
    ...
    if let Some(obj) = value.as_object() {
        ...
        for (key, val) in obj {
            ...
            // ---- Apollo State 模式：key 包含 ":"（如 VisionVideoDetail:{"id":"xxx"}） ----
...
fn extract_video_from_apollo_entry(entry: &Value, photo_id: &str) -> Option<VideoInfo> {
    ...
    // 包含 caption / coverUrl / photoUrl 等字段
